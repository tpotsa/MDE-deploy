...
locals {
  ...
  bigquery_sink_template_vars = {
    ...
    replica_count                     = var.mde_size_details[var.mde_size].bigquery_sink_replicas
    mde_deployment_node_selector      = var.mde_size_details[var.mde_size].mde_deployment_node_selector

    mde_deployment_bigquerysink_container_image_tag = var.mde_deployment_bigquerysink_container_image_tag
  ...
  bigquery_sink = {
    name   = "bigquery-sink"
    labels = ""
    path   = format("%s/%s", var.mde_helm_bigquerysink_chart_path, "bigquery-sink/")
    values = format("%s/%s", var.mde_helm_bigquerysink_chart_path, "bigquery-sink/values.yaml")
  }
}
resource "helm_release" "mde_bigquery_sink" {
  chart     = lookup(local.bigquery_sink, "path", "")
  name      = lookup(local.bigquery_sink, "name", "")
  namespace = kubernetes_namespace.mde_container_namespace.metadata[0].name
  values    = [
    templatefile(lookup(local.bigquery_sink, "values", ""), local.bigquery_sink_template_vars)
  ]
  wait             = false
  timeout          = 900
  disable_webhooks = true
  depends_on       = [
    google_service_account_iam_binding.mde_bigquery_sink_workload_identity_binding,
